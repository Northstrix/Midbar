...
#define TFT_CS    17                                                        // TFT CS  pin is connected to RPI's pin GP16
#define TFT_RST   26                                                        // TFT RST pin is connected to RPI's pin GP26
#define TFT_DC    27                                                        // TFT DC  pin is connected to RPI's pin GP27
                                                                            // SCK (CLK) ---> RPI's pin GP18
                                                                            // MOSI(DIN) ---> RPI's pin GP19
  ...
  while (break_the_loop == false) {
    ...
    if (chosen_lock_screen == 3) {
      for (int i = 0; i < 306; i++) {
        for (int j = 0; j < 77; j++) {
          if (mdb_icon[i][j] == 1)
            tft.drawPixel(i + 7, j + 87, Salt_Lake_City[(i + 7 + k) % 320][j + 87]);
        }
      }
    }
