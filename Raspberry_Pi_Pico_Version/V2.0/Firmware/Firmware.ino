...
EncButton2 < EB_ENC > enc0(INPUT, 0, 1);
EncButton2 < EB_BTN > encoder_button(INPUT, 2);
EncButton2 < EB_BTN > a_button(INPUT, 3);
...
String kderalgs = "h4s863D8sp76bBeb325ksjB8sb48r8izZccgrF1zu9t42K";
int numofkincr = 843;
byte hmackey[] = {"51C62SekhV8F732rm959N4qhB1SH1N39p6QZ9cbO6tpAan761g7v3sXo2KyDuT9Bb2HGWT9J83Ur7u9992YUmaU5Lv2xHt1z2C6P11LKQ41jr535"};
byte des_key[] = {
0xb7,0x19,0xfb,0xd4,0xaa,0x7a,0x1f,0xb0,
0x9c,0xcd,0xff,0x4c,0xe5,0x2d,0x48,0xaa,
0x5e,0xc0,0xf8,0xf4,0x8f,0xf0,0xef,0xf8
};
uint8_t AES_key[32] = {
0xec,0x4a,0xc2,0xf6,
0x9f,0x39,0x8b,0xfd,
...
void display_letters_with_shifting_background(){
  ...
  if (chosen_lock_screen == 1){
    for (int i = 0; i < 306; i++){
      for (int j = 0; j < 77; j++){
        if (mdb_icon[i][j] == 1)
          mvng_bc.drawPixel(i, j, London[(i + 7 + k)%320][j+87]);
      }
    }
  }
