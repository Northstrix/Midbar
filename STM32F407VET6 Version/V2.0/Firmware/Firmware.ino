...
byte read_cards[16] = {
0x64,0x8f,0xff,0xe2,0xbd,0xf8,0xfb,0x59,
0xdc,0x40,0x33,0xf0,0x0f,0x3d,0x06,0x76
};
String kderalgs = "UYoAqs66TD8OLW37VxigL8JpY7lb11Lm3w63c0";
int numofkincr = 809;
...
byte des_key[] = {
0x8d,0xfe,0xba,0xd4,0xed,0xe1,0x3d,0x1f,
0x9b,0xde,0xae,0xfb,0xeb,0x27,0xed,0x5a,
0x9f,0x1a,0xcd,0x5c,0x7c,0xb8,0x67,0x41
};
uint8_t AES_key[32] = {
0x70,0xee,0xac,0x3f,
0xeb,0x1d,0x92,0xb3,
...
byte get_PS2_keyb_and_nunchuk_input() {
...
void fastscroll_get_key_from_ps_keyb(){
   code = keyboard.available();
   if (code > 0) {
     code = keyboard.read();
     //Serial.print("Value ");
  ...
  if (rec_d == false){ // fastscroll_get nunchuk input
    boolean success;
    success = wii_nunchuk.update(); // fastscroll_get new data from the controller
    if (success) {
      if (wii_nunchuk.buttonC() == true) {
        if (pressed_c == false) {
          if (c_functions_as_enter == true){
...
byte fastscroll_get_PS2_keyb_and_nunchuk_input() {
  byte data_from_cntrl_and_keyb;
  while (rec_d == false) {
    delay(4);
    fastscroll_get_key_from_ps_keyb();
  }
  if (rec_d == true) {
    data_from_cntrl_and_keyb = data_from_keyboard;
    rec_d = false;
...
void nintendo_64_and_ps2_keyboard_controller_input() {
  ...
  while (finish_input == false) {
    inp_frm_cntr_and_ps2_kbrd = fastscroll_get_PS2_keyb_and_nunchuk_input();
...
void select_login(byte what_to_do_with_it) {
  ...
  while (continue_to_next == false) {
    byte input_data = get_PS2_keyb_and_nunchuk_input();
    if (input_data > 0) {
...
void display_lock_screen() {
  if ((generate_random_number() % 2) == 0){
    for (int j = 0; j < 240; j++) {
      for (int i = 0; i < 320; i++) {
        tft.drawPixel(i, j, lock_screen_one[i][j]);
      }
    }
  }
  else{
    for (int j = 0; j < 240; j++) {
      for (int i = 0; i < 320; i++) {
        tft.drawPixel(i, j, lock_screen_two[i][j]);
      }
    }
  }
}
