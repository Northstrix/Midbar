...
SoftwareSerial mySerial(PB5, PB6);

#define DATAPIN PB4
#define IRQPIN PB3

#define DATAPIN_ard PB5
#define IRQPIN_ard PB6
...
EncButton2 < EB_ENC > enc0(INPUT, PA0, PA1);
EncButton2 < EB_BTN > encoder_button(INPUT, PB9);
EncButton2 < EB_BTN > a_button(INPUT, PB8);
EncButton2 < EB_BTN > b_button(INPUT, PB7);
...
// Keys (Below)
String kderalgs = "53OxMIO7G5T5mCw2q1KolwzgJp25YJMu37g2i";
int numofkincr = 447;
byte hmackey[] = {"t08WWZD8y569JqIm2Wp5GB356k3ONEk13pyzAcVqk3ea22BZ06GMbJ99E5MFUVXOS3MGk1sbEP1N109K7Yb5bb479N77kkGTLdVWO7gpX4aHDe75m672NJEb"};
byte des_key[] = {
0xcf,0xb1,0x4d,0xbb,0x4b,0x4f,0x8e,0x90,
0x6e,0xaa,0xda,0x50,0x36,0x63,0xf9,0xa2,
0x14,0xcd,0xff,0x38,0x84,0xdd,0x8e,0xd8
};
uint8_t AES_key[32] = {
0xf0,0x9a,0x85,0x8e,
0x93,0xcd,0xce,0x6b,
...
int get_random_number(){
  ...
  /*
  Serial.println("Split received number by digits");
  for (int i = 0; i < inp_from_arduino.length(); i++)
    Serial.println(int(inp_from_arduino.charAt(i)));
  */
  int obtained_value;
  if (inp_from_arduino.charAt(0) != 0 && inp_from_arduino.charAt(1) != 0)
    obtained_value = 16 * getNum(inp_from_arduino.charAt(0)) + getNum(inp_from_arduino.charAt(1));
  if (inp_from_arduino.charAt(0) != 0 && inp_from_arduino.charAt(1) == 0)
    obtained_value = 16 * getNum(inp_from_arduino.charAt(0));
  if (inp_from_arduino.charAt(0) == 0 && inp_from_arduino.charAt(1) != 0)
    obtained_value = getNum(inp_from_arduino.charAt(1));
  if (inp_from_arduino.charAt(0) == 0 && inp_from_arduino.charAt(1) == 0)
    obtained_value = 0;
  ...
  int gen_pseudorandom_nmb = random() % 256;
  /*
  Serial.println("Result");
  Serial.println(obtained_value);
  Serial.println(obtained_value ^ gen_pseudorandom_nmb);
  */
  mySerial.end();
  return obtained_value ^ gen_pseudorandom_nmb;
}
...
void encrypt_iv_for_tdes_aes_blwfsh_serp() {
  ...
  for (int i = 0; i < 10; i++) {
    array_for_CBC_mode[i] = get_random_number();
  }
...
void split_by_eight_bl_aes_serp_aes(char plntxt[], int k, int str_len) {
  ...
  */
  for (int i = 8; i < 16; i++) {
    encr_for_aes[i] = get_random_number();
  }
  /*
  Serial.println("\nFor AES");
  for (int i = 0; i < 16; i++){
...
void encr_AES_bl_aes_serp_aes(char t_enc[]) {
  ...
  */
  ...
  for (int i = 8; i < 16; i++) {
    L_half[i] = get_random_number();
    R_half[i] = get_random_number();
  }
...
void action_for_data_on_sd_card_menu(int curr_pos) {
  tft.setTextSize(2);
  byte sdown = 60;
  if (curr_pos == 0) {
    tft.setTextColor(0xffff);
...
void action_for_data_on_sd_card(String menu_title, byte record_type) {
  tft.fillScreen(0x0000);
  tft.setTextSize(2);
  tft.setTextColor(current_inact_clr);
  disp_centered_text(menu_title, 10);
  curr_key = 0;
  action_for_data_on_sd_card_menu(curr_key);
  disp_button_designation();
...
void display_lock_screen(){
  tft.setTextSize(2);
  disp_centered_text("Midbar STM32F401CCU6", 4);
  for (int i = 0; i < 320; i++){
    for (int j = 0; j < 190; j++){
...
void lock_scr_with_rfid() {
  ...
  tft.fillRect(20, 220, 280, 20, 0x0000);
  disp_centered_text("Put The Card Away", 220);
  delay(1250);
  tft.fillRect(20, 220, 280, 20, 0x0000);
  ...
  /*
  Serial.println(read_card1);
  Serial.println(read_card2);
  Serial.println(read_card3);
  Serial.println(read_card4);
  */
}
...
void loop() {
  ...
  if (a_button.press()) {
    if (curr_key == 0)
      action_for_data_on_sd_card("Logins Menu", curr_key);

    if (curr_key == 1)
      action_for_data_on_sd_card("Credit Cards Menu", curr_key);

    if (curr_key == 2)
      action_for_data_on_sd_card("Notes Menu", curr_key);

    if (curr_key == 3)
      action_for_data_on_sd_card("Phone Numbers Menu", curr_key);
