...
// Keys (Below)
byte read_cards[16] = {
0xef,0x8b,0x0f,0xbf,0x7e,0xf4,0x64,0x9a,
0xa6,0x06,0x8f,0xdb,0xfc,0x76,0x2b,0xad
};
String kderalgs = "hfu7wefIwL6PqNk865Q0r1k0tl";
int numofkincr = 979;
byte hmackey[] = {"9V7d7092Ws0d4T6M5nL0Z9q04DfVI1WV02E2YJV3f4E1Q6t6jMpd8DTd8pTk4xlT9V4HBlsw2dfg88e4Ch6wwsy7mTPACdCS3WGP94HPzmf"};
byte des_key[] = {
0x1f,0x83,0x6c,0xf4,0xeb,0x9c,0xde,0xa0,
0x69,0xda,0x5a,0xdc,0x48,0xb1,0x6f,0x75,
0xcf,0xc8,0x1e,0xda,0xc9,0x7a,0x1b,0xc6
};
uint8_t AES_key[32] = {
0x25,0xe3,0xfd,0xaa,
0x0c,0x4f,0xa3,0x1f,
...
void encrypt_iv_for_tdes_aes_blwfsh_serp() {
  ...
  for (int i = 0; i < 10; i++) {
    array_for_CBC_mode[i] = random() % 256;
  }
...
void set_pass() {
  ...
  for (int i = 0; i < 125; i++) {
    for (int j = 0; j < 39; j++) {
      tft.drawPixel(i + 97, j + 10, handwritten_midbar[i][j]);
    }
  }
  tft.setTextColor(0xffff);
...
void display_lock_screen(){
    ...
    for (int i = 0; i < 320; i++){
      for (int j = 0; j < 160; j++){
        tft.drawPixel(i, j + 40, Denver[i][j]);
      }
    }
/*
  for (int i = 0; i < 306; i++){
...
void setup(void) {
  ...
  tft.setTextSize(2);
  disp_centered_text("Midbar STM32F401CCU6", 8);
  Serial.begin(115200);
