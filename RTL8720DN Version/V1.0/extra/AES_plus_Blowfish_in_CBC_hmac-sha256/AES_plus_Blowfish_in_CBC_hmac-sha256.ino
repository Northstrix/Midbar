...
void encrypt_string_with_AES_Blowfish(String input) {
  back_keys();
  clear_variables();
  encrypt_iv_for_aes_blwfsh();
  SHA256HMAC hmac(hmackey, sizeof(hmackey));
...
void decrypt_string_with_AES_Blowfish(String ct) {
  back_keys();
  clear_variables();
  decrypt_tag = true;
  int ct_len = ct.length() + 1;
...
void loop() {
  ...
  Serial.println("3.Encrypt tag");
  Serial.println("4.Decrypt tag");
  Serial.println("5.Encrypt string with tag");
  Serial.println("6.Decrypt string with tag");
  Serial.println("7.Set encryption algorithm to AES-128");
  Serial.println("8.Set encryption algorithm to AES-192");
  Serial.println("9.Set encryption algorithm to AES-256");
  ...
  if(x == 1){
    ...
    String input;
    while (!Serial.available()) {}
    encrypt_with_AES_Blowfish(Serial.readString());
  ...
  if(x == 2){
     ...
     while (!Serial.available()) {}
     decrypt_with_AES_Blowfish(Serial.readString());
  ...
  if(x == 3){
    ...
    String input;
    while (!Serial.available()) {}
    encrypt_tag_with_AES_Blowfish(Serial.readString());
    Serial.println("Ciphertext:");
    Serial.println(dec_st);
  }
  if(x == 4){
     ...
     Serial.println("Enter ciphertext:");
     while (!Serial.available()) {}
     decrypt_tag_with_AES_Blowfish(Serial.readString());
  ...
  if(x == 5){
    ...
    String input;
    while (!Serial.available()) {}
    encrypt_string_with_AES_Blowfish(Serial.readString());
    Serial.println("Ciphertext:");
    Serial.println(dec_st);
  }
  if(x == 6){
     ...
     Serial.println("Enter ciphertext:");
     while (!Serial.available()) {}
     decrypt_string_with_AES_Blowfish(Serial.readString());
     Serial.println("Plaintext:");
     Serial.println(dec_st);
     bool intgr = verify_integrity();
     if (intgr == true){
