...
public class Editor {
    ...
    public static void gen_keys() {
	    ...
	    disp_rec("byte hmackey_for_session_key[] = {\"");
	    SecureRandom hfssk = new SecureRandom();
	    int num_of_chars_in_hk2 = 99 + hfssk.nextInt(50);
	    for (int i = 0; i < num_of_chars_in_hk2; i++) {
	      SecureRandom rndsky = new SecureRandom();
	        m = rndsky.nextInt(3);
	        m1 = rndsky.nextInt(2);
	        StringBuilder hmckeyfrsssn = new StringBuilder();
	        if (m == 0)
	          hmckeyfrsssn.append((char)(65 + (rndsky.nextInt(26))));
	        if (m == 1)
	          hmckeyfrsssn.append((char)(97 + (rndsky.nextInt(26))));
	        if (m == 2)
	          hmckeyfrsssn.append((char)(48 + (rndsky.nextInt(10))));
	        disp_rec(hmckeyfrsssn.toString());
	    }
	    disp_rec("\"};\n");
	    
	    disp_rec("uint8_t projection_key[32] = {\n");
	    for (int i = 0; i < 8; i++) {
	      SecureRandom rnforsrpprk = new SecureRandom();
	      for (int j = 0; j < 4; j++) {
	        m = rnforsrpprk.nextInt(2);
	        m1 = rnforsrpprk.nextInt(2);
	        disp_rec("0x");
	        StringBuilder aprjkstr = new StringBuilder();
	        if (m == 0)
	          aprjkstr.append((char)(97 + (rnforsrpprk.nextInt(6))));
	        if (m == 1)
	          aprjkstr.append((char)(48 + (rnforsrpprk.nextInt(10))));
	        if (m1 == 0)
	          aprjkstr.append((char)(97 + (rnforsrpprk.nextInt(6))));
	        if (m1 == 1)
	          aprjkstr.append((char)(48 + (rnforsrpprk.nextInt(10))));
	        aprjkstr.append(",");
	        if (i == 7 && j == 3)
	          aprjkstr.setLength(aprjkstr.length() - 1);
	        disp_rec(aprjkstr.toString());
	      }
	      disp_rec("\n");
	    }
	    disp_rec("};\n");
	    
	    disp_rec("uint8_t proj_serp_key[32] = {\n");
	    for (int i = 0; i < 8; i++) {
	      SecureRandom scrfrsrppr = new SecureRandom();
	      for (int j = 0; j < 4; j++) {
	        m = scrfrsrppr.nextInt(2);
	        m1 = scrfrsrppr.nextInt(2);
	        disp_rec("0x");
	        StringBuilder srpprjkstr = new StringBuilder();
	        if (m == 0)
	          srpprjkstr.append((char)(97 + (scrfrsrppr.nextInt(6))));
	        if (m == 1)
	          srpprjkstr.append((char)(48 + (scrfrsrppr.nextInt(10))));
	        if (m1 == 0)
	          srpprjkstr.append((char)(97 + (scrfrsrppr.nextInt(6))));
	        if (m1 == 1)
	          srpprjkstr.append((char)(48 + (scrfrsrppr.nextInt(10))));
	        srpprjkstr.append(",");
	        if (i == 7 && j == 3)
	          srpprjkstr.setLength(srpprjkstr.length() - 1);
	        disp_rec(srpprjkstr.toString());
	      }
	      disp_rec("\n");
	    }
	    disp_rec("};");
	  ...
	  public static void main(String[] args){
	      ...
	      */
		  StringBuilder slt = new StringBuilder();
	        ...
	        JButton genr = new JButton("Generate keys for Midbar V3.0");
